...
internal actual fun print(level: NgLog.LogLevel, tag: String, message: String) {
    val logMessage = when (level) {
        NgLog.LogLevel.DEBUG -> "DEBUG[$tag]: $message"
        NgLog.LogLevel.INFO -> "INFO[$tag]: $message"
        NgLog.LogLevel.WARNING -> "WARN[$tag]: $message"
        NgLog.LogLevel.ERROR -> "ERROR[$tag]: $message"
        NgLog.LogLevel.NONE -> "NONE[$tag]: $message"
    }
    NSLog(logMessage)
}